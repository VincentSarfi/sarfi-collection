...
export default function BuchenOverviewContent({ locale }: { locale: Locale }) {
  const tb = getDict(locale).booking;
  const t = tb.pages;
  ...
  return (
    <div className="min-h-screen bg-cream-50 pt-20">
          ...
          <nav aria-label="Breadcrumb" className="mb-1">
            <ol className="flex items-center gap-2 font-body text-xs text-cream-50/40">
              ...
              <li className="flex items-center gap-2">
                ...
                <span className="text-cream-50/60">{t.overview.crumb}</span>
              </li>
            </ol>
          </nav>
          <h1 className="font-display text-2xl text-cream-50">
            {t.overview.h1.plain}<span className="text-gold-300">{t.overview.h1.gold}</span>
          </h1>
      ...
      <div className="container-site py-8 space-y-10">
        ...
        {/* ── HAUS28 ── */}
        <section>
          <h2 className="font-display text-xl text-forest-900 mb-4">{t.overview.haus28SectionTitle}</h2>
          <Link
              ...
              <div className="p-5 flex flex-col justify-between gap-4 flex-1">
                <div>
                  ...
                  <p className="font-body text-sm text-forest-500 mb-4 leading-snug">
                    {t.overview.haus28Sub}
                  </p>
                  <div className="flex flex-wrap gap-3 text-forest-400 text-xs font-body mb-4">
                    <span className="flex items-center gap-1"><IconUsers size={12} />{t.overview.guestsN(haus28.maxGuests ?? 0)}</span>
                    <span>·</span>
                    <span>{t.overview.bedroomsN(haus28.bedrooms ?? 0)}</span>
                    <span>·</span>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-body text-xs text-forest-400">{tb.labels.from}</span>
                    <span className="font-display text-2xl text-forest-800">{haus28.priceFrom}€</span>
                    <span className="font-body text-xs text-forest-400"> {tb.labels.perNight}</span>
                  </div>
        ...
        <section>
          <h2 className="font-display text-xl text-forest-900 mb-4">{t.overview.schoenblickSectionTitle}</h2>
              ...
              >
                ...
                <div className="relative aspect-[3/2] overflow-hidden">
                  ...
                  {apt.isNew && (
                    <div className="absolute top-3 left-3 px-2 py-0.5 bg-gold-500 rounded-full">
                      <span className="font-body text-xs font-bold text-forest-900 tracking-widest uppercase">{t.overview.newBadge}</span>
                    </div>
                  )}
                  {apt.airbnbRating > 0 && (
                    <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 bg-forest-900/70 backdrop-blur-sm rounded-full">
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-3 text-forest-400 text-xs font-body mb-3">
                    <span className="flex items-center gap-1"><IconUsers size={12} />{t.overview.guestsN(apt.maxGuests)}</span>
                    <span>·</span>
                    <span>{apt.bedrooms} {t.overview.bedroomsShort}</span>
                    <span>·</span>
                    <span>{apt.sqm} m²</span>
                  </div>
                  <div className="flex items-center justify-between">
