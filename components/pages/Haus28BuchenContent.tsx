          ...
          <div>
            <nav aria-label="Breadcrumb" className="mb-1">
              <ol className="flex items-center gap-2 font-body text-xs text-cream-50/40">
                <li>
                  <Link href={localizeHref("/", locale)} className="hover:text-cream-50/70 transition-colors">
                    {t.home}
                  </Link>
                </li>
                {config.breadcrumb.map((crumb) => (
                  <li key={crumb.href} className="flex items-center gap-2">
                    ...
                    <Link href={localizeHref(crumb.href, locale)} className="hover:text-cream-50/70 transition-colors">
                      {crumb.label}
                    </Link>
                  </li>
                ))}
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-body text-sm text-cream-50/70">
                {haus28.airbnbRating} · {haus28.airbnbReviewCount} {t.reviewsWord}
              </span>
            </div>
